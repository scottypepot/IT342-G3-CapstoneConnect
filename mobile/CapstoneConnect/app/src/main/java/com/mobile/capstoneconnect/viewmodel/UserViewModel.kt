...
class UserViewModel : ViewModel() {
    ...
    fun fetchUserProfile(id: Long) {
        ...
        repository.getUserProfile(id).enqueue(object : Callback<UserProfile> {
            override fun onResponse(call: Call<UserProfile>, response: Response<UserProfile>) {
                if (response.isSuccessful) {
                    _userProfile.value = ApiResult.Success(response.body()!!)
                } else {
                    _userProfile.value = ApiResult.Error("Profile failed: ${response.message()}")
                }
            }
    ...
    fun updateUserProfile(id: Long, updates: Map<String, Any>) {
        _userProfile.value = ApiResult.Loading
        repository.updateUserProfile(id, updates).enqueue(object : Callback<UserProfile> {
            override fun onResponse(call: Call<UserProfile>, response: Response<UserProfile>) {
                if (response.isSuccessful) {
                    _userProfile.value = ApiResult.Success(response.body()!!)
                } else {
    ...
    fun fetchPotentialMatches(id: Long) {
        ...
        repository.getPotentialMatches(id).enqueue(object : Callback<List<MatchUser>> {
    ...
    fun updateFirstTimeUser(id: Long, isFirstTime: Boolean) {
        ...
        repository.updateFirstTimeUser(id, isFirstTime).enqueue(object : Callback<Void> {
